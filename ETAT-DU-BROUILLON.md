...
> **Archivé.** Ce fichier documente une étape antérieure (31 août 2026, 7 établissements
> cartographiques). Depuis, le contenu a été fusionné dans `main` et publié en production ; la
> couche établissements est passée à 22 installations et 31 secteurs. Conservé ici comme trace
> historique de la validation de l'étape 5, pas comme état courant du projet.

Ce fichier était le point de reprise officiel du projet pendant son développement sur `brouillon`.
...
## Action nécessaire avant le dépôt GitHub (complétée)

Cette section décrivait les préparatifs requis avant le tout premier dépôt sur `brouillon` :
importer la version officielle dans `main`, puis configurer le secret de protection du workflow.
Les deux sont faits ; `main` et le secret existent depuis. Conservé tel quel pour l'historique.
